  ...
  return (
    <>
      ...
      <button
        ...
        aria-label={isDragging ? '拖动手柄' : localIsOpen ? '关闭设置面板' : '打开设置面板'}
        title={isDragging ? '拖动中...' : '点击切换，拖动移动'}
      >
        {isDragging ? (
          ...
          <div className={`flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-6 bg-background ${isMobile ? 'pb-mobile-nav' : ''}`}>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 ml-3">
                启用后，按 Ctrl+Enter 发送消息而不是 Enter。这对于输入法用户很有用，可以避免误发送。
              </p>
            </div>

            {/* Whisper Dictation Settings - HIDDEN */}
            <div className="space-y-2" style={{ display: 'none' }}>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">语音输入</h4>
